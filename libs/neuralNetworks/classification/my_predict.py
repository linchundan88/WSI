...
from libs.neuralNetworks.classification.my_is_inception import is_inception_model
...
def predict_csv_ensemble(model_dicts, filename_csv,
                         ...
                         dimension='2D', batch_size=64, argmax=True):
...
def predict_one_cube(model, npy_file, model_convert_gpu=True, argmax=False):
    assert os.path.exists(npy_file), 'fine not found!'

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if model_convert_gpu and torch.cuda.is_available():
        model = model.to(device)
    model.eval()

    np_3d_array = np.load(npy_file)  # shape (64,64,64) (H,W,D) float64

    x = np.expand_dims(np_3d_array, axis=-1)  # (H,W,D) -> (H,W,D,C)
    x = x.transpose((3, 0, 1, 2))  # (C,H,W,D)
    x = np.expand_dims(x, axis=0)  # (N,C,H,W,D)
    x = x / 255.  # output float64
    ...
    if argmax:
        predicts = torch.argmax(probs)
        return probs.cpu().numpy(), predicts.cpu().numpy()
    else:
        return probs.cpu().numpy()